...
public class ToolBarBuilder extends Builder {
	...
	public ToolBarBuilder(String nodeName) {
		super(nodeName);
	}
	 ...
	 * 處理流程
	 * 1.取出在<toolbar></toolbar>裡面的Node
	 * 2.透過解析節點方法完成所有的X????物件放在subNodes
	 * 3.將這些subNodes放入產生的XToolBar
	 * </pre>
	 */
	@Override
	public void processNode(List<UIGen> nodeList, Node node) {
		...
		// 建構XToolBar
		XToolBar toolBar = new XToolBar(node, subNodes);
		// 解析所有子節點放入 subNodes
		super.parserNode(subNodes, node.getChildNodes());
		nodeList.add(toolBar);
	}
}
