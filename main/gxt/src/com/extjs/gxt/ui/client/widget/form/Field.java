...
public abstract class Field<D> extends BoxComponent {
	...
	public class FieldMessages {
		...
		/**
		 * Sets the error text to use when marking a field invalid and no
		 * message is provided (defaults to
	 ...
	 * the current value is different than it's original value. The original
	 * value is the value of the field when the field is rendered. Disabled and
	...
	/**
	 * Marks this field as invalid. Validation will still run if called again,
	 * and the error message will be changed or cleared based on validation. To
	 * set a error message that will not be cleared until manually cleared see
	...
	public void markInvalid(String msg) {
		msg = Format.htmlEncode(msg == null ? getMessages().getInvalidText()
				: msg);
		...
		if ("side".equals(messageTarget)) {
			if (errorIcon == null) {
				errorIcon = new WidgetComponent(getImages().getInvalid()
						.createImage());
	 ...
	 * 
	 * @param autoValidate
	 *            true to validate on each key press
	 */
	 ...
	 * (defaults to false).
	 * 
	 * @param fireChangeEventOnSetValue
	 *            true to fire a change event
	 */
	 ...
	 * 
	 * @param inEditor
	 *            true mark this field being in an editor
	 */
	 ...
	 *            the label separator or "" for none
	 */
	...
	/**
	 * Updates the original value of the field. The originalValue is the value
	 * of the field when it is rendered. This method is useful when a form /
	...
	/**
	 * Sets the field's property editor which is used to translate typed values
	 * to string, and string values back to typed values.
	 * 
	 ...
	 *            true to validate on blur, otherwise false
	 */
	...
	public void setValue(D value) {
		...
		this.value = value;
		if (rendered) {
			String v = value == null ? "" : propertyEditor
	...
	/**
	 * Updates the original value of the field. This method is useful when a
	 * form / field is being reused and the originaValue needs to be reset.
	 * 
	 * @param value
	...
	protected void alignErrorIcon() {
		DeferredCommand.addCommand(new Command() {
			public void execute() {
				errorIcon.el().alignTo(getElement(), "tl-tr",
						new int[] { 2, 3 });
			}
		});
	}
	...
	protected void onRender(Element parent, int index) {
		...
		if (this instanceof TextField) {
			...
			if (!tf.getAllowBlank()) {
				String html = "<span style='position:absolute;color:red;left:-8px;z-index:2'>*</span>";
				El star = new El(html);
				star.insertBefore(getInputEl().dom);
			}
		}
		// #############################################
	}
	...
	/**
	 * Subclasses should provide the validation implementation by overriding
	 * this.
	 * 
	 * @param value
