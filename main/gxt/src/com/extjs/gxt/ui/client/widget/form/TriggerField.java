...
public class TriggerField<D> extends TextField<D> {
	...
	public void setEditable(boolean editable) {
		...
		if (rendered) {
			...
			if (GXT.isAriaEnabled()) {
				fromEl.dom.setAttribute("aria-readonly", editable ? "false"
						: "true");
			}
		}
	}
	...
	/**
	 * True to monitor tab key events to force the bluring of the field
	 * (defaults to true).
	 * 
	 * @param monitorTab
	 *            true to monitor tab key events
	 */
	...
	protected void onClick(ComponentEvent ce) {
		if (!readOnly
				&& ((!editable && getInputEl().dom.isOrHasChild(ce.getTarget())) || (trigger.dom
						.isOrHasChild(ce.getTarget())))) {
			onTriggerClick(ce);
		}
		super.onClick(ce);
	}
	...
	protected void onRender(Element target, int index) {
		...
		trigger = new El(GXT.isHighContrastMode ? DOM.createDiv()
				: DOM.createImg());
		...
		// #############################################
		if (GXT.isChrome) {
			input.dom.getStyle().setVerticalAlign(VerticalAlign.TOP);
		}
		// #############################################
