...
public class BorderLayout extends Layout {
  ...
  private void onCollapse(ContentPanel panel) {
    if (panel.getParent() == layoutContainer && fireEvent(Events.BeforeCollapse, createBorderLaoutEvent(panel))) {
      ...
      // 避免當 GXT 事件將 layoutContainer 隱藏，導致後續透過 EventBus 介入操作 BorderLayout時
      // 複製出來的 BorderLayout 為隱藏問題，此問題為 EZo Event 事件與 GXT 事件處理順序問題
      if (layoutContainer instanceof TabItem && !layoutContainer.isVisible()) {
        isVisible = true;
      }
