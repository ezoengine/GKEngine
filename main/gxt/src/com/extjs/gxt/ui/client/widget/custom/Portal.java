...
public class Portal extends ScrollContainer<LayoutContainer> {
  ...
  private int getRowPosition(int col, int y) {
	...
	public Point getInsertXY(int x, int y) {
		startColumns = new ArrayList<Integer>();
		for (int i = 0; i < numColumns; i++) {
			LayoutContainer con = getItem(i);
			int xx = con.getAbsoluteLeft();
			startColumns.add(xx);
		}
		int col = getColumn(x);
		...
		insertCol = col;
		insertRow = row;
		Point p = new Point(insertCol, insertRow);
		return p;
	}

	public void moveDummy(El el, int x, int y) {
		Point p = getInsertXY(x, y);
		el.removeFromParent();
		LayoutContainer lc = getItem(p.x);
		el.insertInto(lc.el().dom, p.y);
	}
}
