...
public class ListView<M extends ModelData> extends BoxComponent {
	 ...
	 * 
	 * @param element
	 *            the element or any child element
	...
	public void refresh() {
		...
		template.overwrite(
				getElement(),
				Util.getJsObjects(collectData(models, 0),
						template.getMaxDepth()));
		all = new CompositeElement(Util.toElementArray(el()
				.select(itemSelector)));
	 ...
	 * 
	 * @param index
	 *            the items data index in the store
	 */
	...
	/**
	 * Sets the display property. Applies when using the default template for
	 * each item's text.
	 * 
	 ...
	 * 
	 * @param enableQuickTip
	 *            true to enable quicktips
	 */
	...
	/**
	 * Sets the view's model processor. The model processor can be used to
	 * provide "formatted" properties to the XTemplate used to render the view.
	 * 
	 * @see ModelProcessor
	 ...
	 * 
	 * @param html
	 *            the html used only for the text of each item in the list
	 */
	...
	protected void onAdd(List<M> models, int index) {
		if (rendered) {
			...
			Element ref = index == 0 ? all.getElement(elements.length) : all
					.getElement(index - 1);
	...
	protected void onBeforeLoad() {
		if (loadingText != null) {
			if (rendered) {
				el().setInnerHtml(
						"<div class='loading-indicator'>" + loadingText
								+ "</div>");
			}
			all.removeAll();
		}
	...
	protected void onRender(Element target, int index) {
		...
		if (template == null) {
			template = XTemplate
					.create("<tpl for=\".\"><div class='x-view-item' " + aria
		...
		if (GXT.isAriaEnabled()) {
			...
			setAriaState("aria-multiselectable",
					mode != SelectionMode.SINGLE ? "true" : "false");
			setAriaRole("listbox");
		}

		sinkEvents(Event.ONCLICK | Event.ONDBLCLICK | Event.MOUSEEVENTS
				| Event.FOCUSEVENTS | Event.ONKEYDOWN);
	}
