...
public class DatePicker extends BoxComponent {
  ...
  public static final int CHINESE_YEAR = 1911;
  public static final int YEAR = 0;

  // 目前日期顯示格式
  private int dateType = YEAR;
  ...
  protected void onRender(Element target, int index) {
    ...
    String[] dn = XConstants.constants.narrowWeekdays();
  ...
  private int getCalculatedStartDay() {
    return startDay != Integer.MIN_VALUE ? startDay : Integer.parseInt(XConstants.constants.firstDayOfTheWeek()) - 1;
  }
  ...
  private void update(DateWrapper date) {
    ...
    if (vd != null && el() != null) {
      ...
      String t = null;
      // 設定顯示月份和年份, 判斷是要用民國年或是西元年
      if (dateType == CHINESE_YEAR) {
    	t = GXT.MESSAGES.datePicker_chineseYear() + (activeDate.getFullYear() - dateType) + GXT.MESSAGES.datePicker_year() + XConstants.constants.standaloneMonths()[month];
      } else if (dateType == YEAR) {
    	t = XConstants.constants.eraNames()[1] + (activeDate.getFullYear() - dateType) + GXT.MESSAGES.datePicker_year() + XConstants.constants.standaloneMonths()[month];
      } else {
    	t = constants.standaloneMonths()[month] + " " + activeDate.getFullYear();
      }
      monthBtn.setText(t);
    }
  }
  ...
  private void updateMPYear(int year) {
    ...
    for (int i = 1; i <= 10; i++) {
      ...
      //顯示可選擇的年份
      td.firstChild().update("" + (y2 - dateType));
